...
class Controller_Product_Media extends Controller_Core_Action
{
	public function gridAction()
	{
		try {
			...
			$layout->addContent('grid', $layout->createBlock('Product_Media_Grid'));
			$layout->render();
		} catch (Exception $e) {
	...
	public function addAction()
	{
		try {
			...
			$add = $layout->createBlock('Product_Media_Add')->setData(['media' => $media]);
			$layout->addContent('add', $add);
			$layout->render();
		} catch (Exception $e) {
	...
	public function operationAction()
	{
		try {
			...
			elseif ($this->getRequest()->getPost('deleted')) {
				...
				if (!Ccc::getModel('Product_Media')->setData(['media_id' => $this->getRequest()->getPost('delete')])->delete()) {
